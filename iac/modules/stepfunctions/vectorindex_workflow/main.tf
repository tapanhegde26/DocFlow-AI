resource "aws_iam_role" "step_function_role" {
  name = "${var.prefix}-vectorindex-sfn-role"
    ...
    Statement = [
      {
        ...
        Resource = [
          var.read_s3_lambda_arn,
          var.chunk_lambda_arn,
          var.embed_lambda_arn,
...
resource "aws_sfn_state_machine" "vector_index" {
  name     = "${var.prefix}-vectorindex"
  role_arn = aws_iam_role.step_function_role.arn

  definition = templatefile("${path.module}/state_machine_definition.json", {
    read_s3_lambda_arn  = var.read_s3_lambda_arn,
    chunk_lambda_arn    = var.chunk_lambda_arn,
    embed_lambda_arn    = var.embed_lambda_arn,
    store_lambda_arn    = var.store_lambda_arn,
