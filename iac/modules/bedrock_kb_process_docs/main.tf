...
resource "aws_iam_role" "bedrock_kb" {
  name               = "${var.prefix}-bedrock-kb-process-docs-role"
...
resource "aws_opensearchserverless_access_policy" "bedrock_kb_access" {
  name        = "${var.prefix}-knwlgbs-pd-access"
  type        = "data"
...
resource "aws_iam_role_policy" "bedrock_kb_policy" {
  name   = "${var.prefix}-bedrock-kb-process-docs-access"
...
data "aws_iam_policy_document" "bedrock_kb_bedrock_access" {
  ...
  statement {
    ...
    resources = [
      var.s3_bucket_arn,
      "${var.s3_bucket_arn}/tagged_processes/*"
    ]
  }
}
...
resource "aws_iam_role_policy" "bedrock_kb_aoss_policy" {
  name = "${var.prefix}-bedrock-kb-process-docs-aoss-access"
  role = aws_iam_role.bedrock_kb.id
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
...
resource "awscc_bedrock_knowledge_base" "bedrock_knowledge_base" {
  name        = "${var.prefix}-bedrock-processdocs-kb"
...
resource "awscc_bedrock_data_source" "tsh-industries_kb_datasource" {
  ...
  name              = "tsh-industries-kb-DataSource"
...
resource "aws_iam_role" "bedrock_sync_lambda" {
  name = "${var.prefix}-bedrock-sync-lambda-role"
...
resource "aws_iam_role_policy" "bedrock_sync_lambda_policy" {
  name = "${var.prefix}-bedrock-sync-lambda-policy"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "arn:aws:logs:${data.aws_region.current.id}:${data.aws_caller_identity.current.account_id}:log-group:/aws/lambda/${var.prefix}-bedrock-sync-lambda*"
      }
    ]
  })
}
...
resource "aws_lambda_function" "bedrock_sync" {
  ...
  function_name    = "${var.prefix}-bedrock-sync-lambda"
