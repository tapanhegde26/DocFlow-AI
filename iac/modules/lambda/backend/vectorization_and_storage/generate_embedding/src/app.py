...
def lambda_handler(event: Dict[str, Any], context) -> Dict[str, Any]:
    ...
    try:
        ...
        if "chunks" in event:
            ...
            process_s3_key = event.get("process_s3_key")
            process_bucket = event.get("process_bucket")
            ...
            process_s3_key = payload.get("process_s3_key")
            process_bucket = payload.get("process_bucket")
            # Also preserve top-level event data
            original_event_data.update({k: v for k, v in event.items() if k != "Payload"})
            logger.info(f"📋 Extracted process_s3_key: {process_s3_key}")
            logger.info(f"📋 Extracted process_bucket: {process_bucket}")
        ...
        response = {
            ...
            **{k: v for k, v in original_event_data.items() 
            if k not in ["original_metadata", "original_llm_tags", "original_tag_categories", "original_tagging_info", "process_s3_key", "process_bucket"]}
        }
