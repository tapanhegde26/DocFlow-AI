...
def lambda_handler(event: Dict[str, Any], context) -> Dict[str, Any]:
    ...
    try:
        ...
        # Validate file is in tagged_processes folder
        if not key.startswith('tagged_processes/'):
            error_msg = f"File is not in the tagged_processes folder: {key}"
            logger.error(error_msg)
            raise ValueError(f"File must be in tagged_processes/ folder: {key}")
        ...
        # Replace the problematic logging section with safe key access
        if 'metadata' in parsed_content:
            ...
            logger.info(f"llm_tags: {parsed_content.get('llm_tags')}")
            logger.info(f"tag_categories: {parsed_content.get('tag_categories')}")
            logger.info(f"tagging_info: {parsed_content.get('tagging_info')}")
            
            # Safe access to nested keys
            process_details = parsed_content.get('process_details', {})
            if isinstance(process_details, dict):
                logger.info(f"mermaid_syntax: {process_details.get('mermaid_syntax', 'Not found')}")
            else:
                logger.info(f"process_details is not a dict: {type(process_details)}")
        ...
        logger.info(f"📋 ✅ OUTPUT: process_s3_key={key}, process_bucket={bucket}")
