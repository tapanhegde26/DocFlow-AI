...
resource "null_resource" "lambda_image_build" {
  ...
  command = <<-EOT
    ...
    docker buildx build --platform linux/amd64 --load -t ${var.ecr_repository_name}:read-sop-np .
    
    # Tag and push
    docker tag ${var.ecr_repository_name}:read-sop-np ${var.ecr_repository_url}:read-sop-np
    docker push ${var.ecr_repository_url}:read-sop-np
  EOT
}
}
...
resource "aws_iam_role" "lambda_exec" {
  name               = "${var.prefix}-vectorization-nonprocess-read-sop-s3"
...
data "aws_iam_policy_document" "lambda_s3_access" {
  statement {
    ...
    resources = [
      "arn:aws:s3:::${var.bucket_name}/non_distinct_processes/*"
    ]
  }
  ...
  statement {
    ...
    condition {
      test     = "StringLike"
      variable = "s3:prefix"
      values   = ["non_distinct_processes/*"]
    }
  }
}

resource "aws_iam_policy" "lambda_s3_read_policy" {
  name        = "${var.prefix}-vectorization-s3-nonprocess-read"
