...
resource "null_resource" "lambda_image_build" {
  ...
  command = <<-EOT
    ...
    docker buildx build --platform linux/amd64 --load -t ${var.ecr_repository_name}:generate-embedding-np .
    
    # Tag and push
    docker tag ${var.ecr_repository_name}:generate-embedding-np ${var.ecr_repository_url}:generate-embedding-np
    docker push ${var.ecr_repository_url}:generate-embedding-np
  EOT
}
}
...
data "aws_ecr_image" "lambda_image" {
  ...
  image_tag       = "generate-embedding-np"
  
  depends_on = [null_resource.lambda_image_build]
}

resource "aws_lambda_function" "generate_embedding" {
  function_name = "${var.prefix}-vectorization-nonprocess-generate-embedding"
...
resource "aws_iam_role" "lambda_exec" {
  name = "${var.prefix}-generate-embedding-nonprocess"
...
resource "aws_iam_policy" "bedrock_invoke_model" {
  name        = "${var.prefix}-bedrock-invoke-nonprocess"
...
resource "aws_iam_policy" "lambda_s3_putobject" {
  name        = "${var.prefix}-lambda-s3--nonprocess-putobject"
