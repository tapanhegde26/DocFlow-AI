...
resource "null_resource" "lambda_image_build" {
  ...
  command = <<-EOT
    ...
    docker buildx build --platform linux/amd64 --load -t ${var.ecr_repository_name}:chunk-sop-np .
    
    # Tag and push
    docker tag ${var.ecr_repository_name}:chunk-sop-np ${var.ecr_repository_url}:chunk-sop-np
    docker push ${var.ecr_repository_url}:chunk-sop-np
  EOT
}
}
...
data "aws_ecr_image" "lambda_image" {
  ...
  image_tag       = "chunk-sop-np"
  
  depends_on = [null_resource.lambda_image_build]
}

resource "aws_lambda_function" "chunk_sop" {
  function_name = "${var.prefix}-vectorization-nonprocess-chunk-and-embed"
...
resource "aws_iam_role" "lambda_exec" {
  name = "${var.prefix}-vectorization-nonprocess-chunk-embed-role"
...
resource "aws_iam_policy" "bedrock_policy" {
  name = "${var.prefix}-bedrock-embed-nonprocess-access"
