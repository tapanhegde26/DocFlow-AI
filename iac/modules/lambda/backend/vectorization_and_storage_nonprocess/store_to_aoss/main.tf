...
resource "null_resource" "lambda_image_build" {
  ...
  command = <<-EOT
    ...
    docker buildx build --platform linux/amd64 --load -t ${var.ecr_repository_name}:store-aoss-np .
    
    # Tag and push
    docker tag ${var.ecr_repository_name}:store-aoss-np ${var.ecr_repository_url}:store-aoss-np
    docker push ${var.ecr_repository_url}:store-aoss-np
  EOT
}
}
...
data "aws_ecr_image" "lambda_image" {
  ...
  image_tag       = "store-aoss-np"
  
  depends_on = [null_resource.lambda_image_build]
}

resource "aws_lambda_function" "store_vector_db" {
  function_name = "${var.prefix}-vectorization-nonprocess-store-vector-db"
...
resource "aws_iam_role" "lambda_exec" {
  name = "${var.prefix}-vectorization-nonprocess-store-vector-db"
...
data "aws_iam_policy_document" "lambda_s3_access" {
  statement {
    ...
    resources = [
      "arn:aws:s3:::${var.prefix}-nonprocess-sop-embedding",
      "arn:aws:s3:::${var.prefix}-nonprocess-sop-embedding/*",
      "arn:aws:s3:::${var.prefix}-nonprocess-sop-embedding/embeddings/*"
    ]
  }
}

resource "aws_iam_policy" "lambda_s3_access" {
  name        = "${var.prefix}-vectorization-nonprocess-lambda-s3-access"
...
resource "aws_iam_policy" "aoss_access" {
  name   = "${var.prefix}-vectorization-nonprocess-aoss-access"
...
resource "aws_opensearchserverless_access_policy" "lambda_data_access" {
  name = "${var.prefix}-lmd-np-dt-access"
  type = "data"
