# modules/pubsub/data_ingestion_topic/main.tf

# Main topic for data ingestion events
resource "google_pubsub_topic" "data_ingestion" {
  project = var.project_id
  name    = "${var.prefix}-data-ingestion-topic"
...
resource "google_pubsub_subscription" "data_ingestion_sub" {
  project = var.project_id
  name    = "${var.prefix}-data-ingestion-sub"
...
resource "google_pubsub_subscription" "data_ingestion_dlq_sub" {
  project = var.project_id
  name    = "${var.prefix}-data-ingestion-dlq-sub"
  topic   = google_pubsub_topic.data_ingestion_dlq.name
